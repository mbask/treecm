...
sampled by B. De Cinti and M. Bascietto:
...
The tilted stem is clearly visible, as is the tree climber.<span
data-label="fig:stonePine1"></span>](stonePine1_low.jpg)
...
Log biomass is computed by Smalian's formula (@laMarca). It is important
...
CM <- centreOfMass(vectors)
summary(CM)
plot(vectors, main = "A stone pine centre of mass under 2x snow load", xlim = c(-8,10), ylim =
c(-12,4))
plot(CM)
plotPolarSegment(210, 10.6, 140, 14.4)
```
...
has a physical meaning @Matth, branches are not usually vertical. As the
...
There's almost a 30% decrease in the force to the plinth if we had the chance of setting the plinth 50m away from tree base. Let's now assess how force to the plinth varies when we move the anchor position along the stem. Remember that the tree centre of mass is m high, and that \texttt{l.stem} is the distance between tree base and the anchor, following the stem, not the anchor height on the ground:
...
in published literature. The dataset `Dst` (@niklas) can be a useful
...
-   `allometryCutini2009`, tested on stone pine trees (not on branches),
    24+ cm diameter, returns biomass, dry weight, @cutini, its results
...
-   `allometryPorte2002`, tested on maritime pine branches, 10- cm
    diameter, returns biomass (dry weight), @porte, its results should
...
[The stone pine measured in \texttt{stonePine1TreeData}, crown not visible. The tilted stem is clearly visible, as is the tree climber.}]![image](stonePine1_low.jpg)
...
ggplot2::ggplot(biomass, aes(x = code, y = biomass)) + 
  geom_bar(aes(fill = allometry), position = "dodge", stat = "identity")

ggplot2::ggplot(biomass, aes(x = diameter, y = biomass)) + 
...
```{r, fig.cap="comparing biomass estimates of `allometryABDC` and `allometryCutini2009` functions to `stonePine2FieldData` after adjusting the latter"}
