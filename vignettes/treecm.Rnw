...
Branch and log biomass has been added to field measures and included in the \testtt{treeData} dataset by a simple call to \texttt{treeData <- treeBiomass(treeData)}.
...
menzionare che deve essere lasso ma non troppo perché l'albero, se cade, ci si deve appoggiare senza strappi dovuti ad una velocità eccessiva che può prendere se l'ancora è eccessivamente lassa.
