...
\subsection{Slenderness coefficient}

The slenderness coefficient of a tree is defined as $SC = \frac{h}{d}$ where $h$ is the height of the tree trunk, and $d$ is the diameter of the tree \cite{Matth}. The SC is a measure of tree stability and is extensively used in tree stability measures carried out by Visual Tree Assessment (VTA). SC in the range [30..50] are considered optimal, whereas SC greater than 50 lead to consider the tree at risk of BUCKLING due to its excessive slenderness.
The author has applied the same concept to tree branches as well. As the branch starts to deviate from the verticality (as most of the branches do) the arm of the moment gets longer, reaching a maximum limit in horizontal branches. The longer the arm, the higher the stress on the branch. In order to estimate the added stress imposed by branch angle we improved Mattheck's formula by adding a component proportional to branch tilt angle: $SC_c = \frac{h}{d} \cdot (1 + cos \beta)$

where $\beta$ is branch tilt angle (i.e. $90^{\circ}$ for a vertical branch, $0^{\circ}$ for an horizontal branch).
When \verb!treeData! object is filled with branches length (not the length of thei projection on the ground, from the tree base to their tip) than $SC_c$ can be computed and plotted:
...
plot.SC(SC, txtcol = "black", main = "Branches coefficient of slenderness", xaxt='n', yaxt = 'n', xlab = "", ylab = "")

par(op)
rm(op)
@

The 2D plot charts branches azimuth as arrows whose length is $SC_c$. The longer the arrows the more slender the branch. Arrows pointing inside the red circle are considered to be stable, whereas longer arrows are considered as risky ($SC_c > 50$). The plot may be a visual clue on the process of branch pruning selection. 
...
\begin{itemize}
  ...
  \item The package does not estimate the position of the centre of mass of tree branches. This position may vary according to foliage mass and its distribution along the branchs, branch tapering, quantity of water in leaves (ie: shaded or lit leaves) \emph{etc}. The position must be fed to the package during data loading, as the variable \verb!branchesCM!. Although going for the safe road, setting a branch centre of mass position on its tip may not be sufficiently precise should one assess wood quality as a function of load balance. Work is under way in order to to model branch load balance
...
\bibitem[Nardi Berti, 1979]{nardi} Nardi Berti, R. 1979 \emph{La struttura anatomica del legno ed il riconoscimento dei legnami italiani di pi\`{u} corrente impiego}. CNR (Firenze), 155p.
...
\bibitem[Mattheck et al., 1993]{Matth} Mattheck, C. and Breloer, H., 1993, \emph{La stabilit\`{a} degli alberi}. Il Verde Editoriale (Milano), 281p.
