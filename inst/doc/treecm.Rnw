...
The slenderness coefficient of a tree is defined as $SC = \frac{h}{d}$ where $h$ is the height of the tree trunk, and $d$ is the diameter of the tree \cite{Matth}. The SC is a measure of tree stability and is extensively used in tree stability measures carried out by Visual Tree Assessment (VTA). $SC$ in the range $30 \leq SC \leq 50$ are considered optimal, whereas $SC > 50$ lead to consider the tree at risk of breaking due to its excessive slenderness.
The authors have applied the same concept to tree branches as well. While $SC$ in vertical trees has a physical meaning \cite{Matth}, branches are not usually vertical. As the branch starts to deviate from the verticality (as most of the branches do) the arm of the moment gets longer, reaching a maximum limit in horizontal branches. The longer the arm, the higher the stress on the branch. In order to estimate the added stress imposed by branch angle we improved Mattheck's formula by adding a component proportional to branch tilt angle:

$SC_c = \frac{l}{d} \cdot (1 + cos \alpha)$

where $\alpha$ is branch tilt angle (i.e. $90^{\circ}$ for a vertical branch, $0^{\circ}$ for an horizontal branch). In vertical branches $SC_c=SC$, in horizontal branches $SC_c=2SC$. As far as we know this is the first attempt to apply the slenderness coefficient to branches. Optimal (safe) branches could be in the range $30 \leq SC_c \leq 70$.

When \verb!treeData! object is filled with branches length (not the length of the projection on the ground, from the tree base to their tip) than $SC_c$ can be computed and plotted:
