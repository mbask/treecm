...
Let's model a snow load that doubles branches weight:

<<exSnow>>=
rows <- substr(row.names(treeData$fieldData),1,1)
treeData$fieldData <- within(
  treeData$fieldData,
  biomass[rows != "L"] <- biomass[rows != "L"] * 2
)
rm(rows)
@
...
treeData$fieldData <- within(
  treeData$fieldData, {
  biomass[((dir > 270 | dir < 90) & rows != "L")]   <- biomass[((dir > 270 | dir < 90) & rows != "L")] / 2
  biomass[((dir >= 90 | dir <= 180) & rows != "L")] <- biomass[((dir >= 90 | dir <= 180) & rows != "L")] * 2  
})
rm(rows)
...
\begin{enumerate}
  \item code a simple code assigned to each log or branch (usually \emph{Lx} for logs, \emph{Bx} for branches)
  \item dir orientation, ie: compass bearing in degrees
  \item dBase diameter of log or branch basal section, in cm
  \item dTip diameter of log or branch tip (always 0 for branches), in cm
  \item length log length (leave it empty in case of branches), in m
  \item tipD distance of the tip of the log or branch to tree base (different from branch length when tree stem is not vertical)
  \item height height of log basal section of height of branch insertion on stem
  \item tilt log or branch tilt from the horizontal plane (eg a vertical branch is tilted by 90$^{\circ}$, an horizontal branch is tilted by 0$^{\circ}$), in degrees (optional, only useful to estimate $z$ coordinate of centre of mass) 
  \item toBePruned boolean to simulate branch pruning
\end{enumerate}
There are a few simple rules to be followed in order to layout a correct CSV file, plese refer to page \pageref{sec:rules}
